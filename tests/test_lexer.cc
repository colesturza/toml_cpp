...
void assert_tokens_equal(const std::string &input,
                         const std::vector<toml::token> &expected_tokens) {
  const std::string processed_input = toml::tests::util::dedent(input);
  toml::Lexer lexer(processed_input);

  for (const auto &expected_tok : expected_tokens) {
    toml::token actual_tok = lexer.next_token();
    ASSERT_EQ(expected_tok, actual_tok);
  }
  ASSERT_EQ(lexer.next_token().type, toml::TokenType::TOKEN_EOF);
}

struct LexerTestCase {
  std::string test_name;
  std::string input;
  std::vector<toml::token> expected;

  friend void PrintTo(const LexerTestCase &tc, std::ostream *os) {
    *os << tc.test_name;
  }
};

TEST(LexerBasicTest, TokenizesEmptyInput) {
  assert_tokens_equal("", {});
}

TEST(LexerBasicTest, IgnoresComments) {
  ...
  assert_tokens_equal(input, expected);
}

TEST(LexerBasicTest, TokenizesSimpleKeyValue) {
...
class LexerKeyTest : public ::testing::TestWithParam<LexerTestCase> {};

TEST_P(LexerKeyTest, TokenizesKeysCorrectly) {
  const auto &test_case = GetParam();
  assert_tokens_equal(test_case.input, test_case.expected);
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const ::testing::TestParamInfo<LexerKeyTest::ParamType>& info) {
